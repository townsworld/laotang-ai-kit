...
export default function TruthTranslatorPage() {
  ...
  // API Key state (tracked for UI updates)
  ...
  const handleTranslate = async () => {
    ...
    if (!currentApiKey) {
      setError("请先点击右上角的钥匙图标设置 Gemini API Key");
      return;
    }
    ...
    try {
      const data = await translateText(inputText, currentApiKey);
      setResult(data);
      ...
      const errorMessage = err instanceof Error ? err.message : "分析失败，请稍后再试。";
  ...
  return (
    <div className="min-h-screen text-slate-800 font-sans selection:bg-indigo-100">
      ...
      <div className="absolute top-6 left-6 z-30">
        <Link
          href="/"
          className="inline-flex items-center gap-2 px-4 py-2 bg-white/80 backdrop-blur-sm border border-slate-200 rounded-full text-slate-600 hover:text-indigo-600 hover:border-indigo-300 transition-all shadow-sm hover:shadow-md group"
        >
