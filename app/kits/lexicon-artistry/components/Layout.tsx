  ...
  return (
    <div className="lexicon-artistry min-h-screen w-full bg-cream relative overflow-hidden text-stone-800 font-sans selection:bg-stone-200 selection:text-stone-900 flex flex-col items-center">
      ...
      <div className="absolute top-6 left-6 z-30">
        <Link
          href="/"
          className="inline-flex items-center gap-2 px-4 py-2 bg-white/80 backdrop-blur-sm border border-stone-200 rounded-full text-stone-600 hover:text-stone-800 hover:border-stone-400 transition-all shadow-sm hover:shadow-md group"
        >
      ...
      <div className="relative z-10 w-full flex flex-col items-center flex-1">
        <header className="w-full py-8 md:py-12 px-6 md:px-12 flex justify-between items-center max-w-7xl mx-auto z-50">
          <button 
            ...
            className="text-left group relative ml-16 md:ml-0"
          >
        ...
        {/* Mobile Navigation */}
        {onNavigate && (
          <nav className="md:hidden flex gap-6 mb-4">
            <button 
              onClick={() => onNavigate('home')}
              className={`text-[10px] uppercase tracking-[0.2em] py-1 border-b transition-all duration-500 ${currentView === 'home' ? 'text-stone-900 border-stone-800' : 'text-stone-400 border-transparent hover:text-stone-600'}`}
            >
                {t.nav.analysis}
            </button>
            <button 
              onClick={() => onNavigate('gallery')}
              className={`text-[10px] uppercase tracking-[0.2em] py-1 border-b transition-all duration-500 ${currentView === 'gallery' ? 'text-stone-900 border-stone-800' : 'text-stone-400 border-transparent hover:text-stone-600'}`}
            >
                {t.nav.gallery}
            </button>
          </nav>
